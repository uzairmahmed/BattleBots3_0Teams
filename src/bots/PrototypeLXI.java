...
public class PrototypeLXI extends Bot {
	...
	public int getMove(BotInfo me, boolean shotOK, boolean specialOK, BotInfo[] liveBots, BotInfo[] deadBots, Bullet[] bullets) {
		...
		if (target != null) {
			...
			if (Math.abs(botHelper.calcDistance(me.getX(), me.getY(), target.getX(), target.getY()) ) > 200) {
	...
	private void update(BotInfo me, boolean shotOK, BotInfo[] liveBots, BotInfo[] deadBots, Bullet[] bullets) {
		...
		for (BotInfo fakeTarget : spoofTargets) {
			if ( (myInfo.getX() <= fakeTarget.getX() + RADIUS && 
				myInfo.getX() >= fakeTarget.getX() - RADIUS && 
				myInfo.getY() <= fakeTarget.getY() + RADIUS && 
				myInfo.getY() >= fakeTarget.getY() - RADIUS) || 
	...
	ArrayList<BotInfo> spoofTargets(BotInfo target, BotInfo[] deadBots) {
		...
		if (!target.getTeamName().equals("OmegaSMH") ) {
			...
			if (Math.abs(botHelper.calcDistance(myInfo.getX(), myInfo.getY(),
					closestGrave.getX(), closestGrave.getY() ) ) >= 71) {
				//not stuck in grave
	...
	private double[] calcDangers(double[] choices, ArrayList<Integer> possibleMoves, Bullet[] bullets, BotInfo me,
		...
		//checks for edges
		//if grave looting, allow the bot to go to edge if grave is on edge
		//otherwise, do not allow bot to go to edge
		if (target.isDead()) {
			double targetX = target.getX();
			double targetY = target.getY();
			// edge checks
			if (me.getY() < (BattleBotArena.TOP_EDGE + 50) && targetY > (BattleBotArena.TOP_EDGE + 50)) {
				// top edge
			...
			if (me.getY() > (BattleBotArena.BOTTOM_EDGE - 50)
					&& (targetY + RADIUS * 2) < (BattleBotArena.BOTTOM_EDGE - 50)) {
				// bottom edge
				dangers[1] += ((me.getY() - (BattleBotArena.BOTTOM_EDGE - 50)) / 25);
				dangers[0] -= ((me.getY() - (BattleBotArena.BOTTOM_EDGE - 50)) / 25);
			}
			if (me.getX() < (BattleBotArena.LEFT_EDGE + 50) && targetX > (BattleBotArena.LEFT_EDGE + 50)) {
				// left edge
			...
			if (me.getX() > (BattleBotArena.RIGHT_EDGE - 50)
					&& (targetX + RADIUS * 2) < (BattleBotArena.RIGHT_EDGE - 50)) {
				// right edge
	...
	private double[] calcDesire(double[] choices, ArrayList<Integer> possibleMoves, BotInfo me, BotInfo target) {
		...
		if (!target.isDead()) {
			...
			if (yDif > -idealDistanceY && yDif < idealDistanceY) {
				// fire left
				desires[6] = -6.5
						+ Math.abs((botHelper.calcDistance(me.getX(), me.getY(), target.getX(), target.getY())) / 20);
			} else {
	...
	public BotInfo[] concat(BotInfo[] a, BotInfo[] b) {
		int aLen = a.length;
		int bLen = b.length;
